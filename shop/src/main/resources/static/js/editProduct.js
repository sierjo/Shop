...
document.addEventListener("DOMContentLoaded", function () {
    ...
    fetch('/strona/userCart/existAllCartItems', {
        ...
        .then(addedProductsInUserCart => {
            // Просмотр элементов в избранном
            console.log("ID избранных товаров:", addedProductsInUserCart);

            // Смена иконки
            addedProductsInUserCart.forEach(id => {
            ...
            if (response.ok) {
                ...
                if (oldIcon)
                    ...
                    <svg id="cart-add${productId}" class="my-custom-cart-icon" width="30" height="29" viewBox="0 0 26.00 25.00" 
                                    ...
                                    <path fill="transparent" fill-opacity="1" stroke="#000000" stroke-opacity="1" stroke-width="1" id="tSvg9d51f881b0" title="Ellipse 2" d="M10.9737 20.934C11.5115 20.934 11.9475 21.3416 11.9475 21.8445C11.9475 22.3473 11.5115 22.755 10.9737 22.755C10.436 22.755 10 22.3473 10 21.8445C10 21.3416 10.436 20.934 10.9737 20.934Z" pointer-events="none" style="transform-origin: 10.97px 21.84px;"/>
                                    <path fill="transparent" stroke="#000000" fill-opacity="1" stroke-width="1" stroke-opacity="1" id="tSvgb02df83168" title="Path 5" d="M9.2688 8.024C8.881 8.0119 8.5873 8.3711 8.6762 8.7488L10.3843 16.0082C10.4509 16.2912 10.7138 16.4836 11.0038 16.4612L22.9532 15.5421C23.2727 15.5175 23.5322 15.2743 23.5776 14.9571L24.3984 9.2115C24.4526 8.832 24.1648 8.4895 23.7816 8.4775Z" pointer-events="none"/>
                                    <path fill="transparent" stroke="#000000" fill-opacity="1" stroke-width="1" stroke-opacity="1" id="tSvg264dda6bb1" title="Path 6" d="M12.5 16.5C12.5 16.5 12 8 12 8M15.5 16.5C15.5 16.5 15 8 15 8M18.5 16C18.5 16 18.5 8 18.5 8M21.5 16C21.5 16 22 8 22 8M9 11C9 11 24 11 24 11M9.5 14C9.5 14 24 13.5 24 13.5" pointer-events="none"/>
                                </svg>`;
            } else {
                // Любая другая непредвиденная ошибка (403, 500 и т.д.)
                alert("Something wrong: " + response.status);
